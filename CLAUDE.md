...
- ❌ **HTTP API 를 가지는 Java 서비스에서 springdoc-openapi 미적용** — 같은 PR 에 의존성 + `OpenApiConfig` + `application.yaml` 의 prod 비활성 override 함께 포함 (아래 "작업 규칙 — 새 서비스 OpenAPI 문서화" 참고)
...
### 새 서비스 OpenAPI 문서화 (의무)

HTTP API (`@RestController`) 를 가지는 Java 서비스가 컨트롤러를 처음 갖게 되는 PR 에서는 **같은 PR 에** 다음을 포함한다. 누락 시 NEVER 위반.

1. **`<service>/build.gradle.kts`** — `implementation("org.springdoc:springdoc-openapi-starter-webmvc-ui:2.7.0")` 추가.
2. **`infrastructure/config/OpenApiConfig.java`** — `identity-service` 의 OpenApiConfig 그대로 복사 후 `title` / `description` / 보안 스키마만 서비스 맥락에 맞게 변경.
3. **`application.yaml`** — `springdoc.swagger-ui.path` (`/swagger`), `api-docs.path` (`/v3/api-docs`) 명시 + multi-document `---` 로 `prod` profile 에서 `springdoc.swagger-ui.enabled: false / api-docs.enabled: false` override.
4. **인증 필요 endpoint** — controller 메서드 또는 클래스에 `@SecurityRequirement(name = "BearerJwt")` (또는 동일 이름의 스키마) 명시.
5. gRPC-only 서비스 (HTTP 진입점 없음) 는 본 의무에서 제외.

운영 노출 정책: `/swagger`, `/v3/api-docs` 는 dev/test/local/e2e 한정. prod 는 application.yaml multi-document override 로 차단됨.
