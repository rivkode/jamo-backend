...
> 마지막 갱신: PR #58 머지 (2026-04-28) — **(트랙 외) D-b-1 contracts: Kafka 이벤트 4종 박제** (`DiaryDeleted` + `SentenceFeedbackRequested/Accepted/Rejected`). PR #36 7종 박제 패턴 정합 (compact constructor + EventFields 검증 + JavaDoc 4섹션). 단위 테스트 4 + ContractsArchitectureTest 통과. **해소된 후속 의무**: diary-domain-policy §11 (DiaryDeleted contracts 미정의) + sentence-feedback-domain-policy §16 (선행 필요 4종) + diary-domain-policy §10 Saga cascade 명세 JavaDoc 명시 (자체 cascade / chatroom soft-delete / platform 정정 3 구독자). **단순화**: SentenceFeedbackRequested 의 diaryId / SentenceFeedbackRejected 의 reason 제외 — 후속 호환 변경 (ADR-0004 §6). code-reviewer APPROVE (Critical/High/Medium 0). **chat-service / diary-service / platform-service 의 Outbox 발행 + Consumer 멱등 패턴 모두 본 4 record 의존 — 코드 슬라이스 PR 시 `:contracts` import 가능 상태로 정렬됨**. 다음: **D-b-2** (chat.proto 갱신 — `TranscribeUserAudio` + `SynthesizeAudio` 신설, D-a-4-eval §17 선행 필요 해소) 후 코드 슬라이스 시리즈 (D-a-4-impl + D-a-5-impl).
>
> (이전 갱신: PR #56 — Phase D-a-5 sentence-feedback PRD 평가). 3 proposed PRD (`request / accept / reject`) 모두 KEEP+FIX. sentence-feedback 도메인 정책 16항목 박제 (`docs/decisions/diary/sentence-feedback-domain-policy.md` 신규, ~470줄). 핵심: **권한 가드 403→404 통일 (FIX, comment·diary·diarychat 정합)** + final 상태 충돌 409 / 라이프사이클 KEEP (REQUESTED→SUGGESTED→ACCEPTED·REJECTED·EXPIRED + REQUESTED→FAILED) / TTL 24h / diaryId NULL 허용 (작성 전 미리보기) / chat-service `RequestSentenceFeedback` Deadline 35s / FAILED → 200+fallback (5xx X) / 입력 50 **code points** + priorSentences max 5 / **tone enum casual/formal/neutral** (catalog §147 후속 항목 해소) / quota 일50/분10 / **`SentenceFeedbackRejected` 이벤트 발행 채택** (PRD Open Item — 학습 신호 가치) / DiaryDeleted Saga 자체 cascade / 보존 90일 + UserDataPurged 즉시 삭제 / 일기 본문 자동 반영 = 클라 책임. **PRD §8 Open Item 9건 모두 해소**. **선행 필요 contracts 4종** (`DiaryDeleted` + `SentenceFeedback*3`) — D-b-1 PR. **🎯 diary-service 전체 PRD 평가 완료** (25 PRD = validation 2 + comment 4 + diary 6 + diarychat 8 KEEP+FIX + 1 DROP + 1 proposed + sentence-feedback 3). 다음: **D-b-1 contracts** (Kafka 이벤트 4종 + 선택적으로 D-a-4-eval 의 chat.proto 갱신 합치기) 후 코드 슬라이스 시리즈 (D-a-4-impl + D-a-5-impl).
