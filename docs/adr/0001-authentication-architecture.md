...
- [x] **PKCE 미지원 클라이언트 fallback 정책** → [ADR-0006 결정 1](0006-oauth-provider-integration.md#결정-1--pkce-적용-정책-per-provider-flag) (2026-04-27)
- [ ] Redis 장애 시 fail-open vs fail-closed
- [x] **state 쿠키 SameSite 속성** → `Lax` + HttpOnly + 운영 Secure=true. PKCE verifier 는 별도 flowSession Redis 보관 ([ADR-0006](0006-oauth-provider-integration.md), 2026-04-27). 코드 적용 + cookie 속성 전체 결정은 [decisions/auth/cookie-policy.md](../decisions/auth/cookie-policy.md) (PR3-c, 2026-04-27)
- [ ] JWKS 캐싱 TTL과 키 회전 주기
- [ ] 서비스간 mTLS 도입 시점
- [x] auth/user/profile 모듈 분리 여부 → **ADR-0002에서 결정**
- [x] **deviceId 출처와 fallback** → `X-Device-Id` 헤더 우선 + `web-{UUID}` fallback ([ADR-0006 결정 2](0006-oauth-provider-integration.md#결정-2--deviceid-출처와-fallback), 2026-04-27)
- [x] **OAuth 가입 시 이메일 중복 처리** → 자동 링크 X, 새 User 등록 ([ADR-0006 결정 4](0006-oauth-provider-integration.md#결정-4--이메일-중복-시-자동-링크-여부-msa-정합성), 2026-04-27)

## 참고
- 관련 ADR: ADR-0002 (서비스 분할 — auth/user 모듈 매핑 확정), [ADR-0006](0006-oauth-provider-integration.md) (OAuth provider 통합 세부 결정)
