 ...
 * <b>위험</b>: 향후 PRIVATE↔PUBLIC 전환 / lines 변경을 구독해야 할 도메인이 등장하면 historical replay
 ...
 *   <li>VO 생성 ({@code DiaryLines} / {@code ImageUrls} / {@code Tags}) — invariant 위반 시 도메인 예외 (개수 422 / 길이·기타 400 매핑).</li>
...
public class UpdateDiaryService {
    ...
    public DiaryView update(UpdateDiaryCommand command) {
        ...
        // VO 생성을 트랜잭션 밖에 두면 비작성자의 invalid body 가 InvalidLineLength/Count (400/422) 로
        ...
        ReadResult result = transactionTemplate.execute(status -> {
            ...
            DiaryLines newLines = new DiaryLines(command.lines());
            ImageUrls newImages = new ImageUrls(command.images());
            Tags newTags = Tags.ofStrings(command.tags());
            diary.update(newLines, newImages, newTags, command.visibility(), command.editorId());
