...
/**
 * 로컬 파일시스템 기반 {@link AudioStorage} 구현 (dev — 운영은 후속 S3).
 *
 * <p><b>path traversal 방어</b>: {@code storedName} 은 단순 파일명이어야 한다 — 경로 구분자/상위 참조를 거부하고,
 * baseDir 내부로 resolve 됨을 normalize 후 재확인한다. (storedName 은 도메인이 UUID 로 생성하지만, load 는
 * 외부 URL path 에서 흘러올 수 있어 방어 필수.)
 */
@Component
@Slf4j
