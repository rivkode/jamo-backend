 ...
 *
 * <p>박제 v2 §2/§8-b: 방 접근은 가드(404), host 검증은 도메인 불변식
 * ({@code DiaryChatRoom.setAiAssistant} → 비호스트 403). 값이 실제로 바뀐 경우에만 롱폴 이벤트 append (멱등).
 */
@Service
...
public class SetAiAssistantService {
    ...
    public ChatRoomView setAiAssistant(SetAiAssistant command) {
        ...
        boolean before = room.aiAssistantEnabled();
        room.setAiAssistant(command.actorUserId(), command.enabled());  // 비호스트 → 403
        DiaryChatRoom saved = roomRepository.save(room);
        if (before != command.enabled()) {
            eventRepository.append(ChatRoomEvent.aiToggleChanged(
                saved.id(), command.actorUserId(), command.enabled(), clock));
        }
