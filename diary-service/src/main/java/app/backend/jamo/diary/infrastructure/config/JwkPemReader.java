...
/**
 * Verify-only PEM 파서 — diary-service 는 access token 검증만 (private key 미사용).
 *
 * <p>identity-service `JwkPemReader` (sign + verify 양쪽) 의 verify 전용 슬림 버전. 두 번째 도메인 controller
 * PR 에서 common-auth-jwt 모듈로 추출 검토 (decisions 박제).
 */
