package app.backend.jamo.platform.architecture;
...
public class ArchitectureTest {
    ...
    static final ArchRule no_import_from_other_services =
        noClasses()
            .that().resideInAPackage("app.backend.jamo.platform..")
            ...
            .as("platform-service 는 다른 Java 서비스 모듈을 import 하지 않는다");

    /** R3 — domain 계층은 Spring/JPA 등 프레임워크에 의존하지 않는다. */
    @ArchTest
    static final ArchRule domain_should_not_depend_on_spring_or_jpa =
        noClasses()
            .that().resideInAPackage("app.backend.jamo.platform.domain..")
            .should().dependOnClassesThat()
            .resideInAnyPackage(
                "org.springframework..",
                "jakarta.persistence..",
