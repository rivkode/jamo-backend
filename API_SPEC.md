...
**AI 어시스턴트 (서버 측 생성)** — 상태: **LIVE** (S4, 2026-06-02)
- 별도 클라이언트 호출 없음. `aiAssistantEnabled=true`인 방에서 사용자 메시지가 들어오면 **서버가 AI 답변 메시지(`source:"ai"`)를 생성**해 폴링 `items`로 전달. (author.username=`AI 어시스턴트`, audioUrl=null)
- 구현: diary-service 가 send 커밋 후 **비동기**로 chat-service `AiAssistantService.GenerateChatResponse`(gRPC) 호출 → chat-service 가 ai-service `Complete`(LLM, gpt-4o-mini) 호출 (ADR-0003). 응답은 롱폴로 전달되므로 send 응답은 유저 메시지만 즉시 반환.
- 실패(LLM 장애/한도) 시 `source:"system"` 안내 메시지를 대신 전달. AI 비용 가드: chat-service 사용자별 rate limit(분산 한도는 후속). prod 진입 전 BLOCK 조건: `docs/decisions/api/chat-grpc-internal-only.md`.
